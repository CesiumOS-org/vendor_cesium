...
func init() {
	android.RegisterModuleType("cesium_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cesiumExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cesiumExpandVariables(ctx, dep_file)
	...
	cmd := cesiumExpandVariables(ctx, String(g.properties.Cmd))
